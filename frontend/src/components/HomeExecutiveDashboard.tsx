...
export default function HomeExecutiveDashboard() {
  ...
  if (isLoading || !data) {
    return (
      <div className="rounded-2xl p-8 text-center" style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(45,75,82,0.12)', color: 'var(--brand-teal2)' }}>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-bold" style={{ color: 'var(--brand-teal)', fontFamily: 'Georgia, serif' }}>
            Dashboard Ejecutivo
          </h2>
          <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full" style={{ background: 'rgba(45,75,82,0.08)', color: 'var(--brand-teal2)' }}>
            Holding consolidado
          </span>
        </div>
        <div className="flex gap-1.5">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full" style={{ background: 'rgba(46,107,180,0.15)', color: 'var(--brand-gold)' }}>Financiero</span>
        </div>
      </div>
      ...
      {f.accounts.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {f.accounts.map(a => (
            <div key={a.code} className="rounded-xl px-4 py-3 flex items-center justify-between" style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(45,75,82,0.1)' }}>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {p.projects.map(proj => (
            <div key={proj.id} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(45,75,82,0.12)' }}>
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  {proj.pendingInspections > 0 && (
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5" style={{ background: 'rgba(46,107,180,0.15)', color: 'var(--brand-gold)' }}>
              ...
              {/* progress */}
              <div className="h-1.5 w-full rounded-full mb-2" style={{ background: 'rgba(45,75,82,0.12)' }}>
