...
const ESTADOS: { value: ItemEstado; label: string; color: string; bg: string }[] = [
  ...
  { value: 'EN_CURSO',  label: 'En curso',  color: 'text-[var(--brand-gold)]',  bg: 'bg-[#0071E3]/15 hover:bg-[#0071E3]/25' },
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={() => setShowForm(v => !v)}
          className="text-[10px] text-[#0071E3]/70 hover:text-[var(--brand-gold)] transition-colors flex items-center gap-1">
        ...
        <div className="bg-slate-50 border border-slate-200/40 rounded-xl p-3 mb-3 space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <select value={providerId}
              ...
              className="col-span-2 bg-white border border-slate-200 text-slate-800 text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#0071E3]/60">
            ...
            {providerId === 'OTRO' && (
              <input type="text" placeholder="Nombre del proveedor / contratista" value={vendor}
                ...
                className="col-span-2 bg-white border border-slate-200 text-slate-800 text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#0071E3]/60 placeholder-slate-400" />
            )}
            <input type="number" placeholder="Monto ($)" value={amount}
              onChange={e => setAmount(e.target.value)}
              className="bg-white border border-slate-200 text-slate-800 text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#0071E3]/60 placeholder-slate-400 font-mono" />
          ...
          <textarea placeholder="Notas..." value={notes} onChange={e => setNotes(e.target.value)} rows={2}
            className="w-full bg-white border border-slate-200 text-slate-800 text-xs px-2.5 py-1.5 rounded-lg focus:outline-none focus:border-[#0071E3]/60 placeholder-slate-400 resize-none" />
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={handleSubmit} disabled={createDoc.isPending}
              className="px-3 py-1 bg-[var(--brand-gold)] hover:bg-[#0077ED] text-white text-xs rounded-lg transition-colors disabled:opacity-50">
          ...
          return (
            <div key={doc.id} className="flex items-center gap-2 bg-white/40 rounded-lg px-3 py-2 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                {doc.amount && <div className="text-[10px] font-mono text-[#0071E3]/80">{formatUSD(doc.amount)}</div>}
              </div>
              {doc.fileUrl && (
                <>
          ...
          <td colSpan={10} className="p-0">
            <div className="ml-10 mr-4 my-2 rounded-lg border border-slate-200 bg-white">
              ...
              <div className="px-3 py-2 flex items-center gap-2 bg-slate-50/50">
                ...
                <button onClick={addSub} disabled={!newSubDesc.trim() || createSub.isPending} title="Agregar subactividad" className="flex items-center px-1.5 py-1 bg-[var(--brand-gold)] hover:bg-[#0077ED] text-white rounded disabled:opacity-40"><Plus className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          </td>
        </tr>
        ...
        <div className="mt-0.5 bg-slate-50/60 rounded-b-xl border border-slate-200/30 border-t-0 overflow-hidden">
          ...
          {showNewForm ? (
            ...
            <button onClick={() => setShowNewForm(true)}
              className="flex items-center gap-2 px-4 py-2 text-xs text-slate-500 hover:text-[var(--brand-gold)] hover:bg-[#0071E3]/5 transition-colors w-full border-t border-slate-100">
