...
import { Upload, FileText, CheckCircle, X, AlertTriangle, Mail, MessageCircle, Download, Trash2, Table2, TrendingDown, ChevronDown, RefreshCw, ClipboardCheck } from 'lucide-react'
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        <button onClick={() => setOpen(o => !o)} className="flex-1 flex items-center gap-3 text-left">
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold font-mono
            ${draw.estado === 'WIRED' ? 'bg-emerald-500/20 text-emerald-400' : draw.estado === 'PENDING' ? 'bg-[#3E6B85]/15 text-[var(--brand-gold)]' : 'bg-slate-200 text-slate-400'}`}>
            {draw.drawNumber}
          </div>
          <div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <button onClick={() => fileRef.current?.click()}
                className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:border-[#3E6B85]/40 text-slate-700 hover:text-slate-900 text-sm rounded-xl transition-all">
              ...
              {file && (
                <button onClick={handleParse} disabled={loading}
                  className="px-4 py-2.5 bg-[var(--brand-gold)] btn-animated hover:bg-[#55809B] text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-50">
          ...
          {parsed && (
            <>
              ...
              <button onClick={handleApply} disabled={!targetDraw}
                className="w-full py-2.5 bg-[var(--brand-gold)] btn-animated hover:bg-[#55809B] disabled:opacity-40 text-white text-sm font-semibold rounded-xl transition-colors">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setShowParse(true)}
            className="flex items-center gap-2 px-4 py-2 bg-[var(--brand-gold)] btn-animated hover:bg-[#55809B] text-white text-sm font-medium rounded-xl transition-colors">
            <Upload className="w-4 h-4" />
            Cargar Draw PDF
          </button>
        </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="px-4 py-2 text-right text-[10px] text-[#3E6B85]/80 uppercase tracking-wider">Saldo holdback</th>
