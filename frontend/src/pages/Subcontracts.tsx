...
const STATUS_BADGE: Record<SubContract['status'], string> = {
  ACTIVO: 'bg-[#C8922A]/15 text-[var(--brand-gold)]',
          ...
          {showPayForm ? (
            <form onSubmit={e => {
              ...
              <button type="submit" disabled={!milestone.trim() || !amount || addPaymentMut.isPending}
                className="px-3 py-1.5 bg-[var(--brand-gold)] hover:bg-[#E0AD4F] text-white text-xs font-semibold rounded-lg disabled:opacity-40">
                Agregar
              </button>
            </form>
          ) : (
