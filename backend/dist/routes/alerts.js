...
router.get('/:projectId/alerts', async (req, res) => {
    try {
        ...
        let budgetLevel = 'ok';
        if (totalBudget > 0 && totalEjecutado > totalBudget)
            budgetLevel = 'critical';
        else if (totalBudget > 0 && totalEjecutado > totalBudget * 0.95)
            budgetLevel = 'warning';
        alerts.push({
            id: 'budget-deviation',
            level: budgetLevel,
        ...
        let desfaseLevel = 'ok';
        if (desfase < -10)
            desfaseLevel = 'critical';
        else if (desfase < -5)
            desfaseLevel = 'warning';
        alerts.push({
            id: 'physical-vs-time',
            level: desfaseLevel,
        ...
        if (saldoHoldback < 50000)
            holdbackLevel = 'critical';
        else if (saldoHoldback < 100000)
            holdbackLevel = 'warning';
        alerts.push({
            id: 'holdback-balance',
            level: holdbackLevel,
        ...
        let sfLevel = 'ok';
        if (costoSFProyectado > benchmark)
            sfLevel = 'critical';
        else if (costoSFProyectado > 185)
            sfLevel = 'warning';
        alerts.push({
            id: 'cost-per-sf',
            level: sfLevel,
